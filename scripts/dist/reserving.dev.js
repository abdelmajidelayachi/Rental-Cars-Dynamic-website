...
var e = "";
var priceCal = 0;
var price = document.getElementById('price-cars');
...
function functionLoad2() {
  ...
  countryName.onchange = function () {
    ...
    citySel.length = 1; //display correct values
...
function functionLoad1() {
  ...
  typeDi.onchange = function () {
    console.log(typeDi.value); //empty powers- and cities- dropdown

    powerSel.length = 1;
    typeOfVehicleSel.length = 1; //display correct values
...
window.onload = start(); // console.log(selectedOp1);

var priceOfReserv = function priceOfReserv(type, typeVehicle, energy) {
  var price = 0;
  ...
  if (type == "Automatique" || type == "Manualle" || type == "Autre") {
    if (typeVehicle == "Motor" || typeVehicle == "Berline" || typeVehicle == "Camion" || typeVehicle == "Citadine" || typeVehicle == "Compact" || typeVehicle == "Utilitaire" || typeVehicle == "Engine") {
      if (energy == "Electrique" || energy == "Hybride" || energy == "Diesel" || energy == "Essence") {
        ...
        var days = document.getElementById('days-nums').value; //end
      ...
      } else alert("select energy");
    } else alert("select type ");
  } else alert("Select diver");
};
